...
namespace Examplinvi
{
    ...
    class Program
    {
        static void Main()
        {
            ...
            {
                Console.WriteLine(args.HttpContent);
            };

            var tweet2s = Search.SearchTweets(new SearchTweetsParameters("#hilary")
            {
                SearchType = SearchResultType.Popular,
            }).Where(x => x.Coordinates != null || x.Place != null);

            var tweets = Search.SearchTweets(new SearchTweetsParameters("#hilary")
            {
                SearchType = SearchResultType.Popular,
                GeoCode = new GeoCode(new Coordinates(37.667637, -122.122164), 100, DistanceMeasure.Kilometers)
            }).Where(x => x.Coordinates != null || x.Place != null);
